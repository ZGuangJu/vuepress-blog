...
hash模式背后的原理是onhashchange事件,可以在window对象上监听这个事件,可以在window监听hash的变化。
...
里边有两个属性newURL和oldURL。可以通过模拟改变hsh的值，动态页面数据。
...
- history api可以分为两大部分，切换和修改，参考MDN等官网。
...
在hash模式下，前端路由修改的是 # 中的信息，而浏览器请求时是不带它玩的，所以没有问题.但是在history下，你可以自由的修改 path ，当刷新时，如果服务器中没有相应的响应或者资源，会分分钟刷出一个404来。
